...
function allowedUsers(): Set<string> {
  return new Set(text(Deno.env.get("TELEGRAM_ALLOWED_USER_IDS") || Deno.env.get("TELEGRAM_ALLOWED_USERS"), 4000)
    .split(",").map(v => v.trim()).filter(v => /^\d+$/.test(v)));
}
function isAllowed(userId: unknown): boolean { const configured = allowedUsers(); return configured.size > 0 && configured.has(String(userId ?? "")); }
...
async function sendMessage(chatId: string | number, message: string, replyMarkup?: unknown) {
  return telegram("sendMessage", { chat_id: chatId, text: message, parse_mode: "HTML", disable_web_page_preview: true, ...(replyMarkup ? { reply_markup: replyMarkup } : {}) });
}
async function sendPhoto(chatId: string | number, photo: string, caption: string, replyMarkup?: unknown) {
  return telegram("sendPhoto", { chat_id: chatId, photo, caption: caption.slice(0, 1024), parse_mode: "HTML", ...(replyMarkup ? { reply_markup: replyMarkup } : {}) });
}
async function answerCallback(callbackId: string, message?: string, showAlert = false): Promise<void> {
  await telegram("answerCallbackQuery", { callback_query_id: callbackId, ...(message ? { text: message.slice(0, 180) } : {}), show_alert: showAlert }).catch(() => undefined);
}

function reviewData(row: any): JsonRecord { return row?.data && typeof row.data === "object" && !Array.isArray(row.data) ? row.data : {}; }
function reviewImage(data: JsonRecord): string {
  const candidates = [data?.imageCuration?.primaryImageUrl, data?.imagemPrincipal, Array.isArray(data?.imagens) ? data.imagens[0] : null].map(v => text(v, 2048));
...
async function sendReviewCard(row: any, overrideChatId?: string | number) {
  const chatId = overrideChatId ?? row.chat_id;
  if (!chatId) throw new Error("REVIEW_CHAT_ID_MISSING");
  const caption = reviewCaption(row), keyboard = reviewKeyboard(String(row.id)), image = reviewImage(reviewData(row));
  if (image) { try { await sendPhoto(chatId, image, caption, keyboard); return; } catch {} }
  await sendMessage(chatId, caption, keyboard);
}

function rotationKeyboard(requestId: string) {
  return { inline_keyboard: [[{ text: "✅ Aprovar substituição", callback_data: `rotation_approve:${requestId}` }], [{ text: "🔁 Buscar outra opção", callback_data: `rotation_retry:${requestId}` }], [{ text: "❌ Cancelar rotação", callback_data: `rotation_cancel:${requestId}` }]] };
}
async function legacyBaselineIds(): Promise<Set<string>> {
  ...
  return new Set((Array.isArray(data) ? data : []).map((r: any) => text(r.product_id)).filter(Boolean));
}
async function isPublicRotationSource(product: any): Promise<{ allowed: boolean; legacyBaseline: boolean }> {
  ...
  const baseline = await legacyBaselineIds(), legacy = baseline.has(String(product.id));
  return { allowed: legacy, legacyBaseline: legacy };
}
async function sendRotationCard(requestId: string, chatId?: string | number) {
  const client = adminClient();
  const { data: request, error } = await client.from("product_rotation_requests").select("id,source_product_id,candidate_product_id,telegram_chat_id,status").eq("id", requestId).maybeSingle();
  ...
  const { data: products, error: productsError } = await client.from("products").select("id,produto,display_title,preco,categoria,imagens,image_curation").in("id", [request.source_product_id, request.candidate_product_id]);
  if (productsError) throw new Error("ROTATION_PRODUCTS_UNAVAILABLE");
  const source = (products || []).find((r: any) => r.id === request.source_product_id), candidate = (products || []).find((r: any) => r.id === request.candidate_product_id);
  ...
  const target = chatId ?? request.telegram_chat_id, image = reviewImage(candidate);
  if (image) { try { await sendPhoto(target, image, message, rotationKeyboard(requestId)); return; } catch {} }
  await sendMessage(target, message, rotationKeyboard(requestId));
}

async function persistUpdate(updateId: number, rawBody: string) { await adminClient().rpc("cerberus_telegram_register_update", { p_update_id: updateId, p_payload_hash: await sha256(rawBody) }); }
async function markUpdate(updateId: number, status: "processed" | "rejected" | "error", code?: string) { await adminClient().rpc("cerberus_telegram_mark_update", { p_update_id: updateId, p_status: status, p_error_code: code || null }).catch(() => undefined); }
function callbackContext(update: any) {
  const q = update?.callback_query;
  return { callbackId: text(q?.id, 256), data: text(q?.data, 256), senderId: String(q?.from?.id ?? ""), chatId: String(q?.message?.chat?.id ?? ""), messageId: String(q?.message?.message_id ?? "") };
}
...
async function handleCallback(update: any) {
  ...
  if (ctx.data.startsWith("cancel_rev:")) {
    ...
    const { error } = await client.rpc("cerberus_telegram_discard_review", { p_review_id: reviewId, p_sender_id: ctx.senderId, p_chat_id: ctx.chatId, p_message_id: ctx.messageId, p_callback_query_id: ctx.callbackId });
  ...
  if (ctx.data.startsWith("rotation_approve:")) {
    ...
    const { data, error } = await client.rpc("cerberus_telegram_apply_rotation", { p_request_id: requestId, p_sender_id: ctx.senderId, p_chat_id: ctx.chatId, p_message_id: ctx.messageId, p_callback_query_id: ctx.callbackId });
    if (error) {
  ...
  if (ctx.data.startsWith("rotation_retry:") || ctx.data.startsWith("rotation_cancel:")) {
    const retry = ctx.data.startsWith("rotation_retry:"), prefix = retry ? "rotation_retry:" : "rotation_cancel:", requestId = ctx.data.slice(prefix.length);
    const { data, error } = await client.rpc("cerberus_telegram_rotation_decision", { p_request_id: requestId, p_decision: retry ? "rotation_retry" : "rotation_cancel", p_sender_id: ctx.senderId, p_chat_id: ctx.chatId, p_message_id: ctx.messageId, p_callback_query_id: ctx.callbackId });
    ...
    await sendMessage(ctx.chatId, retry ? `🔁 <b>NOVA BUSCA SOLICITADA</b>\n\nRequest: <code>${escapeHtml(requestId)}</code>\nEstado: <code>${escapeHtml(data?.status || "searching")}</code>` : `❌ <b>ROTAÇÃO CANCELADA</b>\n\nRequest: <code>${escapeHtml(requestId)}</code>\nA peça atual continua publicada.`);
    return;
  }
  ...
  if (ctx.data.startsWith("product_rotate:")) {
    ...
    const { data: request, error } = await client.from("product_rotation_requests").insert({ source_product_id: product.id, category: product.categoria, status: "searching", requested_by: ctx.senderId, telegram_chat_id: ctx.chatId, metadata: { origin: "telegram-edge", callbackQueryId: ctx.callbackId, sourceWasLegacyBaseline: visibility.legacyBaseline } }).select("id,status").single();
...
async function publicProductsForTelegram(): Promise<any[]> {
  ...
  const baseline = new Set((baselineResult.data || []).map((r: any) => String(r.product_id)));
  return (productsResult.data || []).filter((r: any) => r.ativo === true || baseline.has(String(r.id))).slice(0, 10);
}

async function handleMessage(update: any) {
  const message = update?.message, senderId = String(message?.from?.id ?? ""), chatId = String(message?.chat?.id ?? ""), command = text(message?.text, 2000);
  ...
  if (reviewMatch) {
    ...
    if (error || !data) { await sendMessage(chatId, "⚠️ Review não encontrada."); return; }
    await sendReviewCard(data, chatId); return;
  }
  if (/^\/produtos(?:\s|$)/i.test(command)) {
    const rows = await publicProductsForTelegram();
    const rendered = rows.length ? rows.map((r: any) => `• <a href="${PUBLIC_SITE}/produto/${encodeURIComponent(r.slug || r.id)}">${escapeHtml(r.display_title || r.produto)}</a> · <code>${escapeHtml(r.ref || r.id)}</code>`).join("\n") : "Nenhuma peça pública encontrada.";
...
async function processUpdate(update: any, rawBody: string) {
...
async function internalAuthorized(req: Request): Promise<boolean> {
  const configured = text(Deno.env.get("CERBERUS_INTERNAL_TOKEN"), 512), supplied = text(req.headers.get("authorization")?.replace(/^Bearer\s+/i, "") || req.headers.get("x-cerberus-internal-token"), 512);
  return equalSecret(configured, supplied);
}
async function parseJson(req: Request): Promise<JsonRecord> {
  const raw = await req.text(); if (!raw || raw.length > MAX_BODY_BYTES) throw new Error("INVALID_BODY");
  const body = JSON.parse(raw); if (!body || typeof body !== "object" || Array.isArray(body)) throw new Error("INVALID_BODY"); return body;
}
function normalizeReviewPayload(input: JsonRecord) {
  ...
  const images = Array.isArray(data.imagens) ? data.imagens.map((i: unknown) => text(i, 2048)).filter((i: string) => /^https:\/\//i.test(i)).slice(0, 12) : [];
  const primary = text(data?.imageCuration?.primaryImageUrl || data.imagemPrincipal || images[0], 2048);
  return { ...data, produto: text(data.produto || data.rawTitle, 500), rawTitle: text(data.rawTitle || data.produto, 500), displayTitle: text(data.displayTitle || data.produto || data.rawTitle, 90), categoria: text(data.categoria, 160), preco: Number(data.preco), imagens: images, imageCuration: data.imageCuration && typeof data.imageCuration === "object" ? data.imageCuration : { status: "ready", primaryImageUrl: primary }, link: text(data.link, 2048), normalizedUrl: text(data.normalizedUrl || data.sourceProductUrl, 2048), shopId: text(data.shopId || data?.existingProduct?.shopId, 120), itemId: text(data.itemId || data?.existingProduct?.itemId, 120), descricao: text(data.descricao, 4000) };
}
...
async function createCard(req: Request): Promise<Response> {
  ...
  let body: JsonRecord; try { body = await parseJson(req); } catch { return json({ ok: false, error: "INVALID_BODY" }, 400); }
  const data = normalizeReviewPayload(body), chatId = text(body.chatId || body.chat_id, 64), senderId = text(body.senderId || body.sender_id || [...allowedUsers()][0], 64);
  if (!chatId || !senderId || !isAllowed(senderId)) return json({ ok: false, error: "REVIEW_OWNER_INVALID" }, 400);
  if (!data.produto || !data.displayTitle || !data.categoria || !Number.isFinite(data.preco) || data.preco <= 0 || !data.link || !data.normalizedUrl || !data.shopId || !data.itemId || !reviewImage(data)) return json({ ok: false, error: "REVIEW_DATA_INCOMPLETE" }, 400);
  const reviewId = text(body.reviewId || body.id, 160) || `edge-review-${crypto.randomUUID()}`, now = Date.now();
  const row = { id: reviewId, chat_id: chatId, sender_id: senderId, first_name: text(body.firstName, 160) || null, username: text(body.username, 160) || null, created_at: now, expires_at: now + 86400000, status: "pending", data, updated_at: new Date().toISOString() };
...
async function sendRotationCardEndpoint(req: Request): Promise<Response> {
  if (!await internalAuthorized(req)) return json({ ok: false, error: "UNAUTHORIZED" }, 401);
  let body: JsonRecord; try { body = await parseJson(req); } catch { return json({ ok: false, error: "INVALID_BODY" }, 400); }
  const requestId = text(body.requestId, 80); if (!requestId) return json({ ok: false, error: "REQUEST_ID_REQUIRED" }, 400);
...
async function registerWebhook(req: Request): Promise<Response> {
  if (!await internalAuthorized(req)) return json({ ok: false, error: "UNAUTHORIZED" }, 401);
  const secret = text(Deno.env.get("TELEGRAM_WEBHOOK_SECRET"), 256), supabaseUrl = text(Deno.env.get("SUPABASE_URL"), 2048).replace(/\/+$/, "");
  ...
  const result = await telegram("setWebhook", { url: webhookUrl, secret_token: secret, allowed_updates: ["message", "callback_query"], drop_pending_updates: false });
  return json({ ok: true, webhookUrl, registered: result === true });
}

function routePath(req: Request): string {
  const pathname = new URL(req.url).pathname.replace(/\/+$/, ""), marker = "/cerberus-telegram-gateway", index = pathname.indexOf(marker);
  return index >= 0 ? (pathname.slice(index + marker.length) || "/") : pathname;
}

Deno.serve(async (req: Request) => {
  const path = routePath(req);
  if (req.method === "GET" && path === "/health") return json({ status: "ok", service: "cerberus-telegram-gateway", runtime: "supabase-edge", botConfigured: Boolean(botToken()), allowedUsersConfigured: allowedUsers().size > 0, webhookSecretConfigured: Boolean(text(Deno.env.get("TELEGRAM_WEBHOOK_SECRET"))), internalTokenConfigured: Boolean(text(Deno.env.get("CERBERUS_INTERNAL_TOKEN"))), renderDependency: false, humanGate: "telegram-db-authorization-v1", legacyBaselineAware: true });
  try {
    ...
    const expectedSecret = text(Deno.env.get("TELEGRAM_WEBHOOK_SECRET"), 256), suppliedSecret = text(req.headers.get("x-telegram-bot-api-secret-token"), 256);
    ...
    const rawBody = await req.text(); if (!rawBody || rawBody.length > MAX_BODY_BYTES) return json({ ok: false, error: "INVALID_BODY" }, 400);
    let update: any; try { update = JSON.parse(rawBody); } catch { return json({ ok: false, error: "INVALID_JSON" }, 400); }
