...
const PUBLIC_PRODUCT_COLUMNS = [
  ...
  "oferta_promocional", "display_title", "curator_note",
].join(",");

const RAW_PAYLOAD_MARKERS = [
  "[url final]", "[titulo identificado]", "[preco identificado]",
  "[total imagens oficiais]", "[imagens extraidas]", "[conteudo da pagina]",
];
  ...
  try {
    ...
    if (path.endsWith("/products") || path.endsWith("/cerberus-public-api")) {
      ...
      const products = (Array.isArray(data) ? data : []).map((product: Record<string, unknown>) =>
        containsRawPayloadMarkers(product.descricao) ? { ...product, descricao: "" } : product
      );
