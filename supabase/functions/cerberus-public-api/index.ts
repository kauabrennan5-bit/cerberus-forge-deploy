...
// Query only fields consumed by the shared whitelist. Expanding this select in
// the future still cannot expand the public DTO by accident.
const PUBLIC_PRODUCT_COLUMNS = [
  "id", "ref", "produto", "categoria", "preco", "imagens", "link", "ativo",
  "destaque", "status", "slug", "descricao", "pagina_ponte_url", "created_at",
  "oferta_promocional", "display_title",
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (path.endsWith("/products") || path.endsWith("/cerberus-public-api")) {
      ...
      const products = toPublicProductDTOs(data);
