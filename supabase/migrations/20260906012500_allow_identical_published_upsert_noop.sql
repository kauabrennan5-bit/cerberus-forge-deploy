-- INSERT ... ON CONFLICT runs BEFORE INSERT triggers before conflict detection.
-- The repository may include already-published rows in a batch upsert. Permit only
-- a byte-for-byte-equivalent published row (excluding created_at) to pass as a no-op.
-- Any material difference still requires a fresh publication authorization.
...
begin
  ...
  -- INSERT ... ON CONFLICT fires BEFORE INSERT before conflict detection.
  -- Permit only an exact re-insert of an already published row as a no-op.
  -- Any material difference still requires a fresh publication authorization.
