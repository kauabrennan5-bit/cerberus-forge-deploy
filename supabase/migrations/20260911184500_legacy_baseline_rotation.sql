...
-- These rows are already public in the protected Cloudflare snapshot from this
-- exact main SHA. This records visibility provenance only; it does not create
-- human approval, activate the row, or satisfy the publication authorization
-- trigger for any future publication.
insert into public.catalog_legacy_baseline(product_id,source_sha,metadata)
select id,
       '949fef27ace775f53b98901db616a759ce4cc025',
       jsonb_build_object('provenance','cloudflare-versioned-snapshot','humanApprovalBackfill',false)
...
begin
  ...
  select exists(select 1 from public.catalog_legacy_baseline where product_id=v_source.id)
    into v_source_is_legacy_baseline;
  if v_source.status<>'published' or (v_source.ativo is not true and v_source_is_legacy_baseline is not true) then
    raise exception 'ROTATION_SOURCE_NOT_PUBLIC';
  end if;
  ...
  v_human_evidence := jsonb_build_object(
    ...
    'sourceWasLegacyBaseline',v_source_is_legacy_baseline
  );
  ...
  values(v_source.id,'hide','telegram_rotation',p_request_id::text,jsonb_build_object('operationId',v_operation_id,'legacyBaseline',v_source_is_legacy_baseline),now())
  ...
  return jsonb_build_object(
    'ok',true,'requestId',p_request_id,'sourceProductId',v_source.id,'productId',v_candidate.id,
    'operationId',v_operation_id,'sourceWasLegacyBaseline',v_source_is_legacy_baseline
  );
end;
$$;
