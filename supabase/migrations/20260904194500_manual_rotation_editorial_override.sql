-- Manual curator confirmation is the final editorial authority for Product Rotation.
-- Technical publication invariants remain mandatory: reviewed/non-empty title,
-- a persisted primary HTTPS image + fingerprint, price, category, Shopee identity,
-- affiliate link and a short-lived publication authorization.
-- Only source=product_rotation may bypass automated editorial score/brand/review gates.
...
comment on function public.enforce_product_publication_authorization() is
  'Central publication guard. Explicit manual curator confirmation may override automated editorial gates only for product_rotation; technical invariants remain mandatory.';
