-- Autonomous best-of-lot publication must not be blocked by editorial review state
-- when a category is below its required floor. The application already validates
-- the objective technical invariants before creating a short-lived authorization.
--
-- Keep hard DB invariants for: non-empty display title, real HTTPS primary image
-- with fingerprint, positive price, valid public category, Shopee affiliate link,
-- Shopee source identity, matching primary image and a valid authorization.
-- Only autonomous_curator/recovery authorizations explicitly carrying
-- deficitFallback=true (or the legacy bestOfLotFallback=true) may bypass the
-- reviewed/clean editorial states.
...
begin
  ...
  -- Preserve the existing rotation recovery contract unchanged.
  if tg_op = 'UPDATE'
  ...
  -- Objective technical invariants remain mandatory in every publication mode.
  if nullif(btrim(new.display_title), '') is null then
  ...
  -- Resolve the short-lived authorization before applying editorial-only checks,
  -- so the explicit autonomous deficit fallback can be honored by the DB gate.
  ...
  where ppa.product_id = new.id
    ...
    and (
      ...
      or (
        ...
        and (
          coalesce((ppa.evidence ->> 'deficitFallback')::boolean, false) = true
          or coalesce((ppa.evidence ->> 'bestOfLotFallback')::boolean, false) = true
        )
      )
      or (
  ...
  v_deficit_fallback :=
    v_authorization_source in ('autonomous_curator', 'recovery')
    and (
      coalesce((v_authorization_evidence ->> 'deficitFallback')::boolean, false) = true
      or coalesce((v_authorization_evidence ->> 'bestOfLotFallback')::boolean, false) = true
    );

  -- Editorial reviewed/clean state remains mandatory outside explicit deficit mode.
  if not v_deficit_fallback then
    if new.display_title_status <> 'reviewed' then
      raise exception 'PRODUCT_PUBLICATION_BLOCKED:DISPLAY_TITLE_NOT_REVIEWED';
    end if;

    if new.image_editorial_status <> 'clean'
       or new.image_curation ->> 'status' <> 'ready' then
      raise exception 'PRODUCT_PUBLICATION_BLOCKED:IMAGE_REVIEW_NOT_CLEAN';
    end if;
  end if;
...
comment on function public.enforce_product_publication_authorization() is
  'Central publication guard. Autonomous deficit fallback may bypass editorial reviewed/clean states while objective technical invariants remain mandatory.';
