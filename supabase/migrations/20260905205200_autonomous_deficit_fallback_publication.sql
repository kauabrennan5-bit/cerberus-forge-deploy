...
begin
  ...
  where psi.product_id = new.id and lower(psi.marketplace) = 'shopee' limit 1;
  ...
  where ppa.product_id = new.id
    ...
    and ((ppa.source = 'product_rotation' and coalesce((ppa.evidence ->> 'manualEditorialOverride')::boolean, false) = true)
  ...
  v_deficit_fallback := v_authorization_source in ('autonomous_curator', 'recovery') and (coalesce((v_authorization_evidence ->> 'deficitFallback')::boolean, false) = true or coalesce((v_authorization_evidence ->> 'bestOfLotFallback')::boolean, false) = true);
