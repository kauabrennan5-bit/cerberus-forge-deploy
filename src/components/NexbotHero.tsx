...
export function NexbotHero({ onEnterCatalog }: NexbotHeroProps) {
  ...
  useEffect(() => {
    ...
    const initializeViewer = async () => {
      ...
      renderer.domElement.setAttribute('aria-label', 'Objeto 3D interativo da Cerberus Finds');
      ...
      const ambientLight = new THREE.HemisphereLight(0xd8d0c4, 0x080504, 0.82);
      scene.add(ambientLight);

      const keyLight = new THREE.DirectionalLight(0xf2ede4, 3.8);
      ...
      const rimLight = new THREE.DirectionalLight(0x8c1a1a, 4.4);
      rimLight.position.set(-5, 2.5, -3.5);
      scene.add(rimLight);

      const fillLight = new THREE.PointLight(0x8c1a1a, 1.75, 20, 2);
      ...
      const fitCamera = () => {
        ...
        fittedDistance = Math.max(verticalDistance, horizontalDistance) * 1.1;

        camera.near = Math.max(0.01, fittedDistance / 100);
        camera.far = fittedDistance * 25;
        camera.position.set(size.x * 0.12, size.y * 0.02, fittedDistance);
        camera.updateProjectionMatrix();
        controls.target.set(0, size.y * 0.03, 0);
        controls.minDistance = fittedDistance * 0.76;
        controls.maxDistance = fittedDistance * 1.28;
        controls.update();
      ...
      loader.load(
        ...
        (gltf) => {
          ...
          modelRoot.traverse((child) => {
            ...
            for (const material of materials) {
              ...
              if (material instanceof THREE.MeshStandardMaterial) {
                material.color.setHex(0x47433f);
          ...
          floor = new THREE.Mesh(
            new THREE.CircleGeometry(Math.max(size.x, size.z) * 0.6, 64),
