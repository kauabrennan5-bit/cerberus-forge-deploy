...
// AN — payload candidate mismatch
 ...
 // AP — configuração canônica da Fase 4
 test("AP — runtime N16 reconhece as envs da Fase 4", async () => { const route = await readFile(new URL("../server/routes/publicationN16Routes.ts", import.meta.url), "utf8"); const bootstrap = await readFile(new URL("../server.ts", import.meta.url), "utf8"); assert.match(route, /N16_PHASE4_PROOF_RUN_ID/); assert.match(bootstrap, /N16_PHASE4_FAKE_PROVIDER_MODE/); });
