...
function editorialTitle(product: Product): string {
  const title = product.displayTitle?.trim();
  if (!title) throw new Error(`WEEKLY_EDITORIAL_TITLE_MISSING:${product.id}`);
  return title;
}

function price(product: Product, now: Date): number {
  const offer = validPromotionAt(product.ofertaPromocional, now);
  if (offer) return offer.price;
  return Number(product.preco);
}

function priceLabel(product: Product, now: Date): string {
  const value = price(product, now);
  if (!Number.isFinite(value) || value <= 0)
    throw new Error(`WEEKLY_CANONICAL_PRICE_MISSING:${product.id}`);
...
function imageUrl(product: Product): string {
  ...
  if (image.status !== "ready" || !image.primaryImageUrl)
    throw new Error(`WEEKLY_PRODUCT_IMAGE_MISSING:${product.id}`);
...
export function renderWeeklyNewsletter(
  ...
  const urls = products.map((product, index) =>
    buildWeeklyGoUrl(
  ...
  const heroTitle = editorialTitle(hero);
  const logo = buildNewsletterAssetUrl(
    "assets/newsletter/branding/cerberus-logo-square.png",
  );
