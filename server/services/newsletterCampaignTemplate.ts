...
function protectGmailTextColors(html: string): string {
  const colors = ["#ffffff", "#c0392b"];
  const withFillColor = colors.reduce((output, color) => output
  ...
  return withFillColor.replace(
    /<([a-z][a-z0-9]*)\b([^>]*\bstyle="[^"]*color\s*:\s*#ffffff(?:!important)?[^"]*"[^>]*)>/gi,
    (match, tag: string, attributes: string) => /\bdata-ogsc=/.test(attributes)
      ? match
      : `<${tag}${attributes} data-ogsc="color: #ffffff;" color="#ffffff">`,
  );
}
...
function renderNewsletterHeader(tagline: string): string {
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#141414" style="width:100%;border-collapse:collapse;border:0;background:#141414;background-color:#141414;"><tr><td align="center" bgcolor="#141414" style="padding:0;background:#141414;background-color:#141414;border:0;"><span style="display:inline-block;vertical-align:middle;width:31px;height:31px;background:#c0392b;color:#ffffff;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:31px;font-weight:700;text-align:center;">CF</span><span style="display:inline-block;vertical-align:middle;padding-left:11px;color:#ffffff!important;-webkit-text-fill-color:#ffffff!important;font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:31px;font-weight:700;letter-spacing:2px;">CERBERUS FINDS</span><p style="margin:12px 0 0;color:#ffffff!important;-webkit-text-fill-color:#ffffff!important;font-family:Arial,Helvetica,sans-serif;font-size:10px;line-height:1.4;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;">${escapeHtml(tagline)}</p></td></tr></table>`;
}
