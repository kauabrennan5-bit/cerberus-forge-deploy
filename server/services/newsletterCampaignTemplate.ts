...
export function renderNewsletterCampaign(
  ...
  const htmlUnsubscribe = includeUnsubscribe
    ? `<p style="margin:0;color:#b0b0b0;font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:1.6;">Você recebeu esta mensagem porque autorizou comunicações de marketing do Cerberus Finds. <a href="${escapeHtml(unsubscribeUrl)}" style="color:#ffffff;text-decoration:underline;">Cancelar inscrição</a>.</p>`
...
function protectGmailTextColors(html: string): string {
  const colors = ["#ffffff", "#b0b0b0", "#c0392b", "#ffffff", "#888888"];
